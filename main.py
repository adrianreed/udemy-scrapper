...
def scrap_udemy_coupons():
    ...
    coupons_list = list()
    udemy_coupons = req.get("https://udemycoupons.me/", headers={"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/95.0.4638.54 Safari/537.36"}).text
    udemycoupons_soup = BeautifulSoup(udemy_coupons, 'html.parser')
    grid = udemycoupons_soup.find_all("a", {"rel": "bookmark"}, href=True)
    for item in grid:
        coupons_list.append(item["href"])
    coupons_list=list( dict.fromkeys(coupons_list) )
    for course in coupons_list:
        try:
            course_html = req.get(course, headers={"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/95.0.4638.54 Safari/537.36"}).text
